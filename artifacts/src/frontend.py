...
def azure2policy_helper(role_definitions, role_assignment):
    ...
    for ra in role_assignment:
        for rd in role_definitions:
            # Extract role definition GUID from paths to match regardless of subscription ID
            ra_role_id = ra['properties']['roleDefinitionId'].split('/')[-1]
            rd_role_id = rd['Id'].split('/')[-1]
            if ra_role_id == rd_role_id:
